...
```
Protocolo Extendido con Delay: [START][SLAVE_ID][COMMAND][RELAY][STATE][DELAY_B3][DELAY_B2][DELAY_B1][DELAY_B0][CHECKSUM][END]

Campos:
- START: 0xAA (byte inicio)
- SLAVE_ID: 1-247 (ID del esclavo destino)
- COMMAND: 0x01 (SET_RELAY_WITH_DELAY)
- RELAY: 1-16 (número de relé en esclavo)
- STATE: 0=OFF, 1=ON
- DELAY_B3-B0: Delay en milisegundos (32 bits, big-endian)
- CHECKSUM: XOR de bytes 0-8
- END: 0x55 (byte fin)

Ejemplo: Activar relé 3 por 8000ms
[0xAA][0x01][0x01][0x03][0x01][0x00][0x00][0x1F][0x40][CHECKSUM][0x55]
```

### Respuesta Esclavo → Master:
```
Formato: [START][SLAVE_ID][COMMAND][STATUS][CHECKSUM][END]
- START: 0xAA
- SLAVE_ID: ID del esclavo que responde
- COMMAND: 0x01 (eco del comando recibido)
- STATUS: 1=ACK (éxito), 0=NACK (error)
- CHECKSUM: XOR de bytes 0-3
- END: 0x55

Ejemplo respuesta ACK: [0xAA][0x01][0x01][0x01][CHECKSUM][0x55]
```

### Manejo de Delay en Esclavo:
- **Delay = 0**: Relé se activa permanentemente (sin auto-apagado)
- **Delay > 0**: Relé se activa temporalmente y se apaga automáticamente
- **Timer independiente**: Cada relé tiene su propio timer
- **Cancelación manual**: Apagar relé manualmente cancela el timer
...
### Problema: Esclavo muestra "Comando RS485 incompleto: 7 bytes"
**Causa**: Master usando protocolo antiguo (7 bytes), esclavo esperando nuevo (11 bytes)
**Solución**: 
- Cargar código actualizado en el master
- El esclavo ahora es compatible con ambos protocolos
- Protocolo antiguo usa delay fijo de 5 segundos
- Protocolo nuevo usa delay personalizable del endpoint /activate
